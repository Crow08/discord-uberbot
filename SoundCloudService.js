...
class SoundCloudService {
  ...
  getSongViaUrl(payload) {
    return new Promise((resolve, reject) => {
      request(`http://api.soundcloud.com/resolve?url=${payload}&client_id=${this.clientId}`, (error, response, body) => {
        if (error) {
  ...
  getSongViaSearchQuery(searchstring) {
    return new Promise((resolve, reject) => {
      request(`https://api.soundcloud.com/tracks?q=${encodeURIComponent(searchstring)}&limit=1&client_id=${this.clientId}`, (error, response, body) => {
        const info = JSON.parse(body);
