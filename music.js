...
try {
    ...
    musicbot.playFunction = (msg, suffix, args) => {
      ...
      if (searchstring.includes("youtu.be/") || searchstring.includes("youtube.com/")){
        ...
        if (searchstring.includes("watch") || searchstring.includes("youtu.be/")) {
          ...
          ytdl.getBasicInfo(searchstring, {}, (err, info) => {
            ...
            if (queue.songs.length === 1 || !client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id)) {
          ...
          ytpl(playid, function(err, playlist) {
            ...
            if (playlist.items.length == 1) {
              ...
              msg.channel.send(musicbot.note('note', `Queued ${playlist.items.length} songs.`));
            }
          });
          return;
        } else {
      ...
      new Promise(async (resolve, reject) => {
        ...
        // send message
        var songTitle = res.title.replace(/\\/g, '\\\\').replace(/\`/g, '\\`').replace(/\*/g, '\\*')
                                     .replace(/_/g, '\\_').replace(/~/g, '\\~').replace(/`/g, '\\`');
        var footerMessage = "";
        if (musicbot.requesterName){
          if(client.users.get(res.requester)) {
            footerMessage += `Requested by ${client.users.get(res.requester).username}\n`
          }else{
        ...
        if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
          ...
          try {
            msg.channel.send(`Adding To Queue: **${songTitle}**\n${footerMessage}`)
          } catch (e) {
            console.error(`[${msg.guild.name}] [playCmd] ` + e.stack);
          };
        };
